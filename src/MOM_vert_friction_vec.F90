...
subroutine vertvisc_coef(u, v, h, forces, visc, dt, G, GV, US, CS, OBC)
  ...
  ! Testing
  real :: imask(SZIB_(G))
  real :: zh2d(SZIB_(G), SZK_(GV)+1)
  real :: zcol1_2d(SZIB_(G), SZK_(GV)+1)
  real :: zcol2_2d(SZIB_(G), SZK_(GV)+1)
  real :: z_clear2d(SZIB_(G), SZK_(GV))
  real :: z2_2d(SZIB_(G), SZK_(GV))
  real :: bbl_mask(SZIB_(G), SZK_(GV))
  real :: botfn2d(SZIB_(G), SZK_(GV))
...
!                                       (1.0-forces%frac_shelf_u(I,j)) * a_cpl(I,K))
      ...
      do K=1,nz+1 ; do I=Isq,Ieq
        CS%a_u(I,j,K) = G%mask2dCu(I,j) * min(a_cpl_max, a_cpl(I,K))
      enddo ; enddo
      do k=1,nz ; do I=Isq,Ieq
        CS%h_u(I,j,k) = G%mask2dCu(I,j) * (hvel(I,k) + h_neglect)
      enddo ; enddo
    endif
  ...
  do J=Jsq,Jeq
    ...
    ! XXX: Testing out a mask
    do i=is,ie
      imask(i) = G%mask2dCv(i,J)
    enddo
...
!  nearly massless layers layers ride over the topography.
    if (CS%harmonic_visc) then
      ...
      ! Compute height from bottom layer, zh
      zh2d(is:ie,nz+1) = 0.
      do k=nz,1,-1 ; do i=is,ie ; if (do_i(i)) then
        zh2d(i,k) = zh2d(i,k+1) + h_harm(i,k)
      endif ; enddo ; enddo

      ! Compute heights (relative to surface) in adjacent cells

      zcol1_2d(is:ie,nz+1) = -G%bathyT(is:ie,j) * GV%Z_to_H
      do k=nz,1,-1 ; do i=is,ie ; if (do_i(i)) then
        zcol1_2d(i,k) = zcol1_2d(i,k+1) + h(i,j,k)
      endif ; enddo ; enddo

      zcol2_2d(is:ie,nz+1) = -G%bathyT(is:ie,j+1) * GV%Z_to_H
      do k=nz,1,-1 ; do i=is,ie ; if (do_i(i)) then
        zcol2_2d(i,k) = zcol2_2d(i,k+1) + h(i,j+1,k)
      endif ; enddo ; enddo

      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        z_clear2d(i,k) = max(zcol1_2d(i,k), zcol2_2d(i,k)) + Dmin(i)
      endif ; enddo ; enddo

      ! Untested!
      if (associated(OBC)) then
        do k=1,nz ; do i=is,ie ; if (do_i(i)) then
          if (zi_dir(i) < 0) z_clear2d(i,k) = zcol1_2d(i,k) + Dmin(I)
          if (zi_dir(i) > 0) z_clear2d(i,k) = zcol2_2d(i,k) + Dmin(I)
        endif ; enddo ; enddo
      endif
      ...
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        z_i(i,k) = max(zh2d(i,k), z_clear2d(i,k)) * I_Hbbl(i)
      endif ; enddo ; enddo

      !--- compute hvel ---

      ! Compute the unweighted z2
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        z2_2d(i,k) = max(zh2d(i,k), max(zcol1_2d(i,k),zcol2_2d(i,k)) + Dmin(i)) * I_Hbbl(i)
      endif ; enddo ; enddo

      ! Now apply the interior weight inside the 2xBBL layer
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        z2_wt = merge( &
            max(0.0, min(1.0, zh2d(i,k) * I_Hbbl(i) * I_valBL - 1.0)), &
            1.0, &
            zh2d(i,k) * I_Hbbl(i) < 2.0 * CS%harm_BL_val &
        )
        z2_2d(i,k) = z2_wt * z2_2d(i,k)
      endif ; enddo ; enddo

      ! Apply the BBL de-scaling to points above the BBL
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        z2 = z2_2d(i,k)
        botfn = 1.0 / (1.0 + 0.09 * z2 * z2 * z2 * z2 * z2 * z2)
        botfn2d(i,k) = merge(1.0, botfn, zh2d(i,k) * I_Hbbl(i) < CS%harm_BL_val)
      endif ; enddo ; enddo

      ! Mask out the values where flow is from large to small h (convergent)
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        botfn2d(i,k) = merge(botfn2d(i,k), 0.0, v(i,J,k) * h_delta(i,k) > 0)
      endif ; enddo ; enddo

      ! Construct hvel from the BBL weight, botfn
      do k=1,nz ; do i=is,ie ; if (do_i(i)) then
        hvel(i,k) = (1.0 - botfn2d(i,k)) * h_arith(i,k) + botfn2d(i,k) * h_harm(i,k)
      endif ; enddo ; enddo
...
!        CS%a_v(i,J,K) = min(a_cpl_max, forces%frac_shelf_v(i,J)  * max(a_shelf(i,K), a_cpl(i,K)) + &
      ...
      do K=1,nz+1 ; do i=is,ie
        CS%a_v(i,J,K) = G%mask2dCv(i,J) * min(a_cpl_max, a_cpl(i,K))
      enddo ; enddo
      do k=1,nz ; do i=is,ie
        CS%h_v(i,J,k) = G%mask2dCv(i,J) * (hvel(i,k) + h_neglect)
      enddo ; enddo
    endif
...
!      h_ml, &  ! The mixed layer depth [H ~> m or kg m-2].
  ...
  ! Testing
  real :: i_mask(SZIB_(G))
  real, dimension(SZIB_(G),SZK_(GV)+1) :: h_shear_2d, botfn_2d
  ...
  ! NOTE: We can eventually use the i-k mask as an argument but for now we
  ! reconstruct it from do_i and work_on_u
  i_mask(:) = merge(1., 0., do_i(:))
...
!  surface mixed layer contributions to the vertical viscosity.
  ...
  if ((GV%nkml>0) .or. do_shelf) then
    do k=2,nz ; do i=is,ie
      !if (do_i(i)) Kv_tot(i,K) = CS%Kv
      Kv_tot(i,K) = i_mask(i) * CS%Kv
    enddo ; enddo
  else
    I_Hmix = 1.0 / (CS%Hmix + h_neglect)
    do i=is,ie ; z_t(i) = h_neglect*I_Hmix ; enddo
  ...
  if (associated(visc%Kv_shear)) then
    ...
    ! NOTE: Compute Kv_add everywhere, then mask during Kv_tot update
    if (work_on_u) then
      !do K=2,nz ; do i=is,ie ; if (do_i(i)) then
      !  Kv_add(i,K) = 0.5*(visc%Kv_shear(i,j,k) + visc%Kv_shear(i+1,j,k))
      !endif ; enddo ; enddo
      ! NOTE: This runs like shit, maybe because of j-index accessing
      do K=2,nz ; do i=is,ie
        Kv_add(i,K) = 0.5 * (visc%Kv_shear(i,j,k) + visc%Kv_shear(i+1,j,k))
      enddo ; enddo
      if (do_OBCs) then
        !do I=is,ie ; if (do_i(I) .and. (OBC%segnum_u(I,j) /= OBC_NONE)) then
        do I=is,ie ; if (OBC%segnum_u(I,j) /= OBC_NONE) then
          if (OBC%segment(OBC%segnum_u(I,j))%direction == OBC_DIRECTION_E) then
      ...
      !endif ; enddo ; enddo
      do K=2,nz ; do i=is,ie
        Kv_tot(i,K) = Kv_tot(i,K) + i_mask(i) * Kv_add(i,K)
      enddo ; enddo
    else
      !do K=2,nz ; do i=is,ie ; if (do_i(i)) then
      ...
      if (do_OBCs) then
        !do i=is,ie ; if (do_i(i) .and. (OBC%segnum_v(i,J) /= OBC_NONE)) then
        do i=is,ie ; if (OBC%segnum_v(i,J) /= OBC_NONE) then
          if (OBC%segment(OBC%segnum_v(i,J))%direction == OBC_DIRECTION_N) then
            do K=2,nz ; Kv_add(i,K) = visc%Kv_shear(i,j,k) ; enddo
          elseif (OBC%segment(OBC%segnum_v(i,J))%direction == OBC_DIRECTION_S) then
  ...
  ! TODO: We can mask Kv_tot and rely it on masking a_cpl because this is the
  !   last appearance of Kv_tot.  But overall it's not a good approach since
  !   Kv_tot could make a future appearance.

  !! v2 - move a_cpl outside
  ...
  !    if (r > bbl_thick(i)) then
  !      !h_shear = ((1.0 - botfn) * r + botfn*bbl_thick(i)) + h_neglect
  !      h_shear_2d(i,k) = ((1.0 - botfn) * r + botfn*bbl_thick(i)) + h_neglect
  !    else
  !      !h_shear = r + h_neglect
